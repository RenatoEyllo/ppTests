...
READ_REQ="\{scenarioId:1,action:1413,social=1,radius:1000,lat:-22.904562,lng:-43.181677,version:%221306%22\}"
...
do
	...
	ab -A tomcat:tomcat -b 1024 -c ${NUM_THREADS} -n ${NUM_OPS} -r -v 4 "http://${SERVER}/mobile?param=${REQUEST}" 2>&1 | tail -${LINES_TAKEN} >> ${OUTPUT_FILE} 
	echo "ab -A tomcat:tomcat -b 1024 -c ${NUM_THREADS} -n ${NUM_OPS} -r -v 4 "http://${SERVER}/mobile?param=${REQUEST}" 2>&1 | tail -${LINES_TAKEN} >> ${OUTPUT_FILE} "
